...
public class UserServiceClient {
    ...
    /**
     * Creates a user record in the user service after successful Keycloak
     * registration
     */
     ...
     */
    public Map<String, Object> createUser(String userId, String username, String email, String firstName,
            String lastName) {
        try {
            ...
            logger.error("Client error creating user in User Service: {} - {}", e.getStatusCode(),
                    e.getResponseBodyAsString());
            return Map.of("error", "User creation failed", "message", e.getResponseBodyAsString(), "status",
                    e.getStatusCode().value());
        } catch (HttpServerErrorException e) {
            logger.error("Server error creating user in User Service: {} - {}", e.getStatusCode(),
                    e.getResponseBodyAsString());
            return Map.of("error", "User service error", "message", "Internal server error in user service", "status",
                    e.getStatusCode().value());
