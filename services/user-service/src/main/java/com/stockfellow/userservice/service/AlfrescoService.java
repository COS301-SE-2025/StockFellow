...
public class AlfrescoService {
    ...
    private void authenticateWithAlfresco() {
        try {
            ...
            logger.error("Error during Alfresco authentication", e);
        }
    }
    
    /**
     * Uploads a document to Alfresco
    ...
    public String uploadDocument(MultipartFile file, String userId, String documentType) throws IOException {
        ...
        try {
            ...
            // Upload the document
            String documentId = uploadFileToFolder(file, userFolderId, documentType);
            
            logger.info("Document uploaded successfully to Alfresco. DocumentId: {}, UserId: {}", 
                       documentId, userId);
            ...
            throw new IOException("Failed to upload document to Alfresco", e);
        }
    }
        ...
        try {
            ...
            Map<String, Object> folderRequest = Map.of(
                ...
                "properties", Map.of(
                    ...
                    "cm:description", "Document storage for user ID: " + userId
                )
            );
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                logger.error("Failed to create user folder. Status: {}", response.getStatusCode());
    ...
    private String getDocumentLibraryId() {
        try {
            ...
            logger.error("Error getting document library ID", e);
            return "-root-";
        }
    }
     ...
     */
    private String uploadFileToFolder(MultipartFile file, String folderId, String documentType) throws IOException {
        ...
        ByteArrayResource fileResource = new ByteArrayResource(file.getBytes()) {
            ...
            public String getFilename() {
                return generateFileName(file.getOriginalFilename(), documentType);
            }
        };
        ...
        String properties = String.format(
            "{\"cm:title\":\"%s\",\"cm:description\":\"Document type: %s, Uploaded: %s\"}",
     ...
     */
    private String generateFileName(String originalFilename, String documentType) {
    ...
    public byte[] downloadDocument(String documentId) throws IOException {
        try {
            ...
            logger.error("Error downloading document from Alfresco. DocumentId: {}", documentId, e);
